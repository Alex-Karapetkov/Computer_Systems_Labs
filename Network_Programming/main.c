  ...
  if (webfile == NULL)
    {
      ...
      // get server list for specified domain and protocol
      struct addrinfo *server_list
          = get_server_list (domain, protocol, tcp, ipv6);

      // error checking
      if (server_list == NULL)
        {
          server_list = get_server_list (domain, protocol, tcp, false);
          char *server_info = serv_string (server_list);
          char *transport_proto = "UDP";

          if (server_list->ai_socktype == SOCK_STREAM)
            {
              transport_proto = "TCP";
            }

          char *ipv4_addr = addr_string (server_list);

          printf ("%s %s: %s IPv6 ::ffff:%s\n", domain, protocol,
                  transport_proto, ipv4_addr);

          server_list = server_list->ai_next;
          free (ipv4_addr);
          free (server_info);
          freeaddrinfo (server_list);
        }

      // traverse through linked list of addresses and print each one along
      // with server info
      struct addrinfo *server = NULL;
      for (server = server_list; server != NULL; server = server->ai_next)
        {
          char *server_info = serv_string (server);
